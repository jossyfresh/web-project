          ...
          {el.resource.map((res, index) => {
            return (
              <Link
                ...
                className={cn(
                  "flex justify-between items-center pr-5 rounded-md",
                  {
              ...
              >
                ...
                <div className="text-sm opacity-75">{res.type}</div>
              </Link>
            );
          })}
        </Accordion.Panel>
  ...
  return (
    <div className="flex justify-end lg:w-[100%] no-scrolbar gap-5 h-full">
      ...
      {openUnit!["resource"][resource_id]["type"] == "quiz" && (
        <Quiz
          quiz={course_material[unit_id][resource_id]}
          link={`/quiz/${unit_id}/${resource_id}`}
        />
      )}
      {openUnit!["resource"][resource_id]["type"] == "file" && (
        <h6>Files here</h6>
      )}
      <div className="lg:w-[30%] sticky right-0 bottom-0 border-gray-100 border shadow-md p-5 lg:space-y-6 space-y-3 flex-col lg:font-extralight mt-5 lg:mt-0">
        <Accordion defaultValue={openUnit?.title}>
          {lessons_accordion}
        </Accordion>
      </div>
    </div>
